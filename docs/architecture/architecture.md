...
1. **`private.handle_new_auth_user()`** — AFTER INSERT trigger on `auth.users` (AFTER, not BEFORE: `public.users.id` holds an FK to `auth.users(id)`, so the auth row must exist within the transaction first), SECURITY DEFINER: normalizes email to lowercase; requires provider-verified email; requires domain **exactly equal** (case-insensitive) to the configured domain — split on the final `@`, full-string compare, never `LIKE`/`endsWith` (EC-A3); on failure `RAISE EXCEPTION` → the entire auth transaction rolls back (no auth user, no session). On success inserts the `public.users` row in the same transaction (`ON CONFLICT DO NOTHING` — EC-A4).
